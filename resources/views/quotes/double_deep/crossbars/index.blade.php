    ...
    <div class="container w-full bg-white p-3 rounded-xl shadow-xl">
        <div class="row m-3">
            ...
            <div  class="row bg-white p-4 shadow-lg rounded-lg">
                {!! Form::open(['method'=>'POST','route'=>['double_deep_crossbars.calc']]) !!}
                    ...
                    <div class="col-sm-4 col-xs-12">
                        <div class="form-group p-2">
                            ...
                            <select name="piece" class="js_select2 inputjet w-full text-xs uppercase">
                                @foreach ($Crossbars as $row)
                                    <option value="{{$row->id}}"@if (old('piece')==$row->type) selected @endif>{{$row->type}}</option>
                                @endforeach
                            </select>
                            <x-jet-input-error for='piece' /><br>
                        </div>
                        <div class="form-group p-2">
                            <x-jet-label value="* Requiere Conector" />
                            <select name="conector" class="js_select2 inputjet w-full text-xs uppercase">
                                <option value="0"@if (old('conector')=='0') selected @endif>No</option>
                                <option value="4"@if (old('conector')=='4') selected @endif>Sí</option>
                            </select>
                        </div>
